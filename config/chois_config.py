...
class CHOISObjectTrajectoryConfig(ArgumentParser):
    def __init__(self):
        super().__init__()
        ...
        self.traj_dataset_configs.add_argument('--trajectory_length', default=200, type=int, help='Total length of trajectory segments extracted')
        self.traj_dataset_configs.add_argument('--history_fraction', default=0.3, type=float, help='Fraction of trajectory_length to use for history (e.g., 0.3 for history/future split)')
        ...
        # === CHOIS Diffusion Model Configuration ===
        self.chois_configs = self.add_argument_group('CHOIS Diffusion Model')
        self.chois_configs.add_argument('--d_feats', default=12, type=int, help='Object trajectory dimension [xyz + 9D rotation matrix]')
        self.chois_configs.add_argument('--d_model', default=512, type=int, help='Transformer model dimension')
        self.chois_configs.add_argument('--n_head', default=8, type=int, help='Number of attention heads')
        self.chois_configs.add_argument('--n_dec_layers', default=6, type=int, help='Number of decoder layers')
        self.chois_configs.add_argument('--d_k', default=64, type=int, help='Dimension of keys in transformer')
        self.chois_configs.add_argument('--d_v', default=64, type=int, help='Dimension of values in transformer')
        ...
        self.diffusion_configs.add_argument('--diffusion_loss_type', type=str, default='l1', choices=['l1', 'l2'], help='Loss function for diffusion training')
        ...
        self.diffusion_configs.add_argument('--objective', type=str, default='pred_x0', choices=['pred_noise', 'pred_x0'], help='Diffusion objective: predict noise or clean signal')
        self.diffusion_configs.add_argument('--beta_schedule', type=str, default='cosine', choices=['linear', 'cosine'], help='Noise schedule for diffusion')
        self.diffusion_configs.add_argument('--p2_loss_weight_gamma', type=float, default=0., help='P2 loss weight gamma parameter')
        self.diffusion_configs.add_argument('--p2_loss_weight_k', type=float, default=1., help='P2 loss weight k parameter')

        # === CHOIS BPS Encoder Configuration ===
        ...
        self.sparse_cond_configs.add_argument('--waypoint_interval', default=30, type=int, help='Frame interval for waypoints in sparse conditioning (for chois_original strategy)')
        self.sparse_cond_configs.add_argument('--use_initial_pose', default=True, action='store_true', help='Use initial pose as condition')
        self.sparse_cond_configs.add_argument('--use_final_position', default=True, action='store_true', help='Use final position as condition')
        self.sparse_cond_configs.add_argument('--use_waypoint_xy', default=True, action='store_true', help='Use xy waypoints as conditions')
        ...
        self.text_configs.add_argument('--text_embedding', action='store_true', default=False, help='Enable text/category embedding')
        ...
        self.train_configs.add_argument('--save_path', type=str, default='chkpoints_chois/', help='Directory to save checkpoints')
        ...
        self.train_configs.add_argument('--first_batch', action='store_true', default=False, help='Debug mode: only process the first batch of each epoch (for debugging NaN issues and quick testing)')
        
        # === Wandb Configuration ===
        self.wandb_configs = self.add_argument_group('WandB')
        self.wandb_configs.add_argument('--wandb_project', type=str, default="CHOIS_Object_Trajectory", help='Wandb project name')
        ...
        self.eval_configs.add_argument('--output_path', default='results_chois/', type=str, help='Directory to save evaluation outputs')
    ...
    def get_configs(self):
        ...
        # Validate trajectory_length vs diffusion max_timesteps
        max_timesteps = args.trajectory_length + 1
        if hasattr(args, 'max_timesteps'):
            args.max_timesteps = max_timesteps
        else:
            # Add this attribute if it doesn't exist
            setattr(args, 'max_timesteps', max_timesteps)
...
if __name__ == '__main__':
    chois_config = CHOISObjectTrajectoryConfig()
    print(chois_config.get_configs())
